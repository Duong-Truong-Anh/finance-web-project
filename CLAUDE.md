...
1. **Token before value.** Every color, type, motion, and breakpoint value is a Carbon token. No raw hex (`#161616`). No ad-hoc font sizes. Use `var(--cds-text-primary)`, `@include type.type-style('label-02')`. The full token discipline lives in `docs/05_design_system_spec.md` § 1.
   - **Spacing is special in this build (ADR 011):** Carbon ships pre-compiled (ADR 002), so the `--cds-spacing-*` scale is **not** a runtime CSS variable — `var(--cds-spacing-*)` in inline styles silently resolves to **0px**. Apply spacing via Carbon layout components (`<Stack gap>`, `<Grid>`/`<Column>`, whose gaps compile to real values), or a documented literal `rem` equal to the token for the rare standalone border/padding/min-size case (e.g. `padding: '3rem' /* --cds-spacing-09 */`). Never `var(--cds-spacing-*)`, and never arbitrary px (`padding: 18px`). Color/type tokens (`var(--cds-text-primary)`, `cds--type-*`) remain mandatory and DO resolve.
...
## Console error monitoring during dev

Three surfaces emit errors. All three must be clean before a UI task is marked done.

1. **Next.js dev terminal** (`bun run dev` output). Server-side errors, build warnings, hot-reload failures, route handler exceptions.
2. **Browser console** (DevTools → Console on the touched route). Client-side React errors, third-party script errors, fetch failures, hydration mismatches.
3. **Playwright `attachErrorGuard`** (`e2e/fixtures/seed.ts`). Captures `pageerror` and `console.error` during e2e runs; enforced empty in `afterEach`.
